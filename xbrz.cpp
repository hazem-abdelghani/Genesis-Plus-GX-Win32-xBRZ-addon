...
// ----------------------------------------------------------------------------
// MODIFIED for the Genesis Plus GX Win32 frontend. The scaling algorithm is
// unchanged; only the following differ from the copy bundled in PPSSPP
// (ext/xbrz), which itself derives from Zenju's HqMAME sources:
//   * build-system includes removed (ppsspp_config.h); config.h renamed to
//     xbrz_config.h so it cannot collide with the frontend's own config.h
//   * the 64 MB colour-distance table is a malloc'd buffer with explicit
//     prepareTables()/releaseTables() instead of a function-local static
//     std::vector, so the table exists only while xBRZ is in use and the
//     program needs no C++ runtime library
// See ../xbrz/README-xbrz.txt.
// ----------------------------------------------------------------------------
...
float* g_distBuffer = nullptr; //[MODIFIED] was a std::vector inside a function-local static; see note at top of file

struct DistYCbCrBuffer //30% perf boost compared to distYCbCr()!
{
public:
	//[MODIFIED] the table is built by xbrz::prepareTables(), which xbrz::scale() calls first
	static bool build()
	{
		if (g_distBuffer) return true;

		float* buffer = static_cast<float*>(std::malloc(sizeof(float) * 256 * 256 * 256)); //consumes 64 MB memory; using double is only 2% faster, but takes 128 MB
	...
	static void destroy()
	{
		std::free(g_distBuffer);
		g_distBuffer = nullptr;
	}
	...
	static double dist(uint32_t pix1, uint32_t pix2)
	{
		...
		return g_distBuffer[(((r_diff + 255) / 2) << 16) | //slightly reduce precision (division by 2) to squeeze value into single byte
							  (((g_diff + 255) / 2) <<  8) |
							  (( b_diff + 255) / 2)];
	}
};
...
bool xbrz::prepareTables() { return DistYCbCrBuffer::build(); } //[MODIFIED]
void xbrz::releaseTables() { DistYCbCrBuffer::destroy(); }     //[MODIFIED]
...
{
	if (!DistYCbCrBuffer::build()) return; //[MODIFIED] no table (out of memory): leave trg untouched
